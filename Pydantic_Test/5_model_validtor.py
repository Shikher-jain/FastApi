...
class Patient(BaseModel):
    ...
    def validate_emergency_contact(model):
        ...
        if age >60 and 'emergency_contact' not in contact_details:
            raise ValueError("Patients above 60 years old must have an emergency contact")
...
def update_data(patient: Patient):
        print(f"Updating data for {patient.name} with age {patient.age}, weight {patient.weight}, height {patient.height}, married {patient.married}, allergies {patient.allergies}, contact details {patient.contact_details}")
